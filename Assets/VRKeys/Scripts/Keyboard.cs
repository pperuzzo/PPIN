...
namespace VRKeys {
	...
	public class Keyboard : MonoBehaviour {
		...
		public ButtonController[] controllers = new ButtonController[0];
		...
		public void Disable () {
			...
			for (int i = 0; i < 5; i++)
			{
				controllers[i].InteractableStateChanged.RemoveAllListeners();
				controllers[i].transform.parent.gameObject.SetActive(false);
			}
        ...
        {
			var res = GameManager.Instance.proteinsInfo.Where(x => x.Name.ToLower().StartsWith(text.ToLower())).Take(5).ToList();
            ...
            {
				texts[i].text = res[i].Name;
				controllers[i].gameObject.name = res[i].Id;
				controllers[i].InteractableStateChanged.AddListener(DisplayNetwork);
				controllers[i].transform.parent.gameObject.SetActive(true);
            }
        }

		public void DisplayNetwork(InteractableStateArgs obj)
		{
			bool inActionState = obj.NewInteractableState == InteractableState.ActionState;
			if (inActionState)
			{
				proteinNetwork.gameObject.SetActive(true);
				proteinNetwork.DisplayNetwork(obj.Interactable.gameObject.name);
				ResetKeyboard();
		...
		private IEnumerator SetupKeys () {
			...
			for (int i = 0; i < layout.row1Keys.Length; i++) {
				...
				key.shifted = false;
				key.transform.GetComponentInChildren<ButtonController>().InteractableStateChanged.AddListener(key.OnPress);
				key.Init (obj.transform.localPosition);

				obj.name = layout.row1Keys[i];
				obj.SetActive (true);
